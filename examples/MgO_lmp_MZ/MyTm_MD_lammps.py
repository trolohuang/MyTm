...
write(lammps_data, atoms, format="lammps-data", atom_style="charge")
...
pair_coeff	1 1 38868.5 0.2066 43.773
pair_coeff	2 2 43469.3723 0.2140 5.0840
pair_coeff	1 2 12655.4 0.2089 36.798
...
fix myNVT all nve
dump mydump all custom 100 {dump_file} id type x y z
...
with open("lammps.output","w") as log:
	subprocess.run(["/opt/intel/oneapi/mpi/2021.17/bin/mpirun","-n","6","lmp_mpi", "-in", lmp_in], stdout=log, stderr=log)


# ============================
# 5. Convert LAMMPS dump to XDATCAR and get thermodynamic parameters
# ============================
symbles_oputput = {1:"Mg",2:"O"}
...
#os.system("rm ./XDATCAR_MyTm")

#for i in range(len(atoms_list)):
	#write(xdatcar_file, atoms_list[i], format="vasp",append=True)
